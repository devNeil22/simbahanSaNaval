<?php
    ...
    if(isset($_POST['search'])){
        ...
        $query = "SELECT * FROM users JOIN christening ON users.u_id = christening.u_id WHERE users.First_Name LIKE '{$search}%' OR users.Last_name LIKE '{$search}%'";
            ...
            while($data = mysqli_fetch_assoc($exe)){ ?>
                <tr>
                    <td>
                        ...
                        <?php }?>
                        <a href="./profile.php?id=<?=$data['u_id']?>" class="user" data-user="<?=$data['Last_name']?> <?=$data['First_Name']?>"> <?=$data['Last_name']?> <?=$data['First_Name']?></a>
                    </td>
                    <td><?=$data['Mobile']?></td>
                    ...
                    <td>
                        ...
                        <a  href="../christining/trans-info.php?id=<?=$data['c_id']?>" class="btn btn-primary"><i class="fa-solid fa-eye"></i></a>
                        <?php if($data['status'] == 0){?>
                         <button class="confirm btn btn-success" data-id="<?=$data['c_id']?>"><i class="fa-solid fa-thumbs-up"></i></button>
                        <?php }else{ ?>
                            <button class="cancel btn btn-danger" data-id="<?=$data['c_id']?>"><i class="fa-solid fa-ban"></i></button>
                        <?php }?>
                    </td>
                </tr>
        <?php }
